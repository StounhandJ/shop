...
class SantehnikParser extends Parser
{
    ...
    public function __construct(string $department_url)
    {
        ...
        if ($this->host != $matches[1]) {
            throw new InvalidSiteException("Неверный сайт");
        }
        $this->department = $this->getOrCreateDepartmentIfNoExist(
            $this->getDepartmentEName($department_url),
            $this->getDepartmentName($department_url)
        );
    ...
    private function getCategories(string $department_url): array
    {
        ...
        foreach ($a as $category) {
            ...
            $data = [];
            $data["category"] = $this->getOrCreateCategoryIfNoExist(
                $matches[1],
                $category->innertext,
                $this->department
            );
    ...
    private function getProducts($sectionId, $totalCnt, $page = 1)
    {
        return $this->postJson(
            $this->product_url,
            ["sectionId" => $sectionId, "perpage" => $totalCnt, "PAGEN_1" => $page]
        )["data"];
    }
